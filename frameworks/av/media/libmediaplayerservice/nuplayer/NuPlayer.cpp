...
status_t NuPlayer::instantiateDecoder(bool audio, sp<Decoder> *decoder) {
    ...
    *decoder = audio ? new Decoder(notify) :
	...
	ALOGD("instantiateDecoder audio %d time %lld",audio,systemTime(SYSTEM_TIME_MONOTONIC) / 1000);
    int64_t durationUs;
    if (mDriver != NULL && mSource->getDuration(&durationUs) == OK) {
        sp<NuPlayerDriver> driver = mDriver.promote();
        if (driver != NULL) {
...
int  NuPlayer::Wifidisplay_get_TimeInfo(void* ptr,int64_t *start_time,int64_t *audio_start_time)
{
	...
	if(mNuPlayer->wifidisplay_flag!=0x1234)
	{
		...
		return mNuPlayer->mSource->Wifidisplay_get_TimeInfo(start_time,audio_start_time);
}
